...
void AModuleAndPluginCharacter::SpawnTestActor()
{
	if (UWorld* World = GetWorld())
	{
		FVector SpawnLocation = GetActorLocation() + GetActorForwardVector() * 200.f;
		FRotator SpawnRotation = FRotator::ZeroRotator;
		ATestActor* MyActor = World->SpawnActor<ATestActor>(ATestActor::StaticClass(), SpawnLocation, SpawnRotation);
		...
		if (MyActor)
		{
			if (GEngine)
			{
				GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Green, TEXT("SpawnTestActor Spawned"));
			}
		}
	}
}
